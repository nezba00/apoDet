...
for path, filename in zip(image_paths, filenames):
    ...
    strdst_df.rename({'t': 'frame'}, inplace=True, axis=1)
    tracked_features = tp.link_df(strdst_df, 10, memory=7)
    ...
    tracked_features.rename(
        {'frame': 't', 'particle': 'track_id'}, 
         inplace=True, axis=1
    )
    ...
    tracked_features.to_csv(merge_df_path, index=False)
    ...
    tracked_masks = convert_obj_to_track_ids(gt_filtered, tracked_features)
...
track_lengths = tracked_masks["track_id"].value_counts()
...
track_sizes = tracked_masks.groupby("track_id")["track_id"].transform('size')
merged_df_long = tracked_masks[track_sizes >= TRK_MIN_LEN].copy()
